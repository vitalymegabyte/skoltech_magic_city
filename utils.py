...
#### Augmentations
def shift_image(img, shift_pnt, borderMode=cv2.BORDER_CONSTANT, borderValue=0):
...
def rotate_image(image, angle, scale, rot_pnt, inter=cv2.INTER_LINEAR, borderMode=cv2.BORDER_CONSTANT, borderValue=0):
    rot_mat = cv2.getRotationMatrix2D(rot_pnt, angle, scale)
    result = cv2.warpAffine(image, rot_mat, (image.shape[1], image.shape[0]), flags=inter, borderMode=borderMode, borderValue=borderValue) #INTER_NEAREST BORDER_REFLECT_101
    return result
