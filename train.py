...
parser.add_argument("--checkpoint", default='convnextv2_base_wPre_256_e04_full')  
...
def train_epoch(current_epoch, seg_loss, cce_loss, model, optimizer, scaler, train_data_loader, amp_autocast=suppress):
    ...
    for __i, sample in enumerate(iterator):
        with torch.cuda.amp.autocast(enabled=args.amp, dtype=torch.bfloat16): #, dtype=torch.bfloat16): amp_autocast(): float16
            imgs = sample["img"].cuda(non_blocking=True)
...
if __name__ == '__main__':
    ...
    snap_to_load = 'convnextv2_base_256_e04_0_pre_best'
    ...
    best_score = 0
    for epoch in range(start_epoch, 6): #start_epoch
